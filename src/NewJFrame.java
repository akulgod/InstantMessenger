...
public class NewJFrame extends javax.swing.JFrame {
    ...
    Statement stmt = null;
    ResultSet rs = null;
    String query, mailfrom = null , username, pass, mail;
    int userid=0;
    ...
    private void initComponents() {
        ...
        msgdtlspg.addWindowListener(new java.awt.event.WindowAdapter() {
            public void windowActivated(java.awt.event.WindowEvent evt) {
                msgdtlspgWindowActivated(evt);
            }
        });
    ...
    private void loginbtn_loginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_loginbtn_loginActionPerformed
        ...
        username = usrnmtxt_login.getText();
        pass = new String(usrpass_login.getPassword());
     
  if (username.equals("admin") && (pass.equals("admin")))//msgpagelogin
    {
            msgdtlspg.setVisible(true);
     } 
  else if(username.equals("test") && (pass.equals("test"))){
   welcome.setVisible(true);
               login.dispose();}
  else               //userpagelogin
     {
       try { Class.forName("java.sql.DriverManager");
             con=(Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/Project","root","tiger");
             Statement stmt=(Statement) con.createStatement();
                query = "select userid, password, email, username, name from userdtls where username =" + "'" + username + "'" + ";";
                rs = stmt.executeQuery(query);
                rs.next();
                String password = rs.getString("password");
                 
            if (pass.equals(password)) /*login successfull*/
               { 
                    ...
                    /**/
                    useridlabel_accsttng.setText("" + rs.getInt("userid"));
                    username_welcome.setText(rs.getString("username"));
                    txtnml_accsttng.setText(username);
    ...
    private void createuserbtn_signupActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_createuserbtn_signupActionPerformed
        // TODO add your handling code here:
      username = inputusrname_signup.getText();
      String  password = new String(inputpass_signup.getPassword()), 
                name = inputnametxt_signup.getText(),
                email = username,
                retypepassword =   new String(inputretypepass_signup.getPassword());
      ...
      if(username!=null && name!=null  && password!=null)
      {
       int x;
        try {Class.forName("java.sql.DriverManager");
            ...
            query = "select count(*) from userdtls where username='"+username+"'";
            rs = stmt.executeQuery(query);
            rs.next();
            ...
            {
                query = "select max(userid)+1 from userdtls";
                rs = stmt.executeQuery(query);
                rs.next();
                userid = rs.getInt("max(userid)+1");
                usridlbl_signup.setText("User  ID : " + userid);
               ...
               stmt.executeUpdate(query);
                JOptionPane.showMessageDialog(null, "User Account Created successfully"
                        +'\n'+ " Please note down your user ID: "+ userid
                        +'\n'+"and your email:"
       ...
       msgtextarea_inbox.removeAll();
        ...
        try {  
            ...
            query = "Select msgid, senderid , recieverid, time, msg, status , reciever , sender , sendermail "
                    + "from msgdtls where recieverid= " + userid +
                    " AND recieverdel = 'no' "  +";";
            rs = stmt.executeQuery(query);
           ...
           /****************************************/
            if (status1 != 0) 
            {
                JOptionPane.showMessageDialog(null, "you have recieved " + status1 + " new message");
            }
    ...
    {   
            ...
            Statement stmt=(Statement) con.createStatement();
            query = "Select msgid, senderid , recieverid, time, msg, status , reciever , sender , sendermail "
                    + "from msgdtls where senderid= " + userid +
                    " AND senderdel = 'no' "  +";";
            rs = stmt.executeQuery(query);
    ...
    private void sendbtn_writeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sendbtn_writeActionPerformed
      ...
      Class.forName("java.sql.DriverManager");
            ...
            query = "select userid, username , email from userdtls where email= '" + mailto + "'";
            rs = stmt.executeQuery(query);
            rs.next();
            reciever = rs.getString("username");/*reciever*/
            recieverid = rs.getInt("userid");/*reciever id*/
    ...
    private void Updatel_accsttngActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Updatel_accsttngActionPerformed
        ...
        try {Class.forName("java.sql.DriverManager");
             con=(Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/Project","root","tiger");
             Statement stmt=(Statement) con.createStatement();
            query = "select userid, password, email from userdtls where username =" + "'" + username + "'" + ";";
            rs = stmt.executeQuery(query);
            rs.next();
            //check old password
...
Class.forName("java.sql.DriverManager");
     ...
     if (rs.getInt("status") == 0) 
            ...
            {
         sentmsgarea_sent.setText(rs.getString("reciever") + " has notread the message yet"+'\n'+"Sent on : " + '\n' + rs.getString("time") + '\n'+ br + "Message:" + '\n' + rs.getString("msg"));
    ...
    private void statsbtn_welcomeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_statsbtn_welcomeActionPerformed
        ...
        int  avgcharsent ,avgcharrecieved;
         try {Class.forName("java.sql.DriverManager");
          ...
          query="select count(*), sum(length(msg)) from msgdtls where senderid = "+ userid+ ";";
          rs= stmt.executeQuery(query);
          rs.next();
       //max msg send
       ...
       //avg char in msg sent
        ...
        query="select count(*), sum(length(msg))  from msgdtls where recieverid= "+ userid+";";
 ...
 if( msg_id!=null){
    ...
    private void msgdtlspgWindowActivated(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_msgdtlspgWindowActivated
        // TODO add your handling code here:
    }//GEN-LAST:event_msgdtlspgWindowActivated
